...
async def cmd_start(message: types.Message):
    ...
    welcome_text = "Привет! Я бот для работы с курсами валют ЦБ РФ."
...
async def cmd_question(message: types.Message):
    ...
    if rates:
        if currency_code in rates:
            ...
            await message.answer(f"📊 Курс валюты:\n\n{result}{get_commands_text()}")
        else:
            currencies_list = get_all_currencies_list(rates)
            await message.answer(
                f"❌ Валюта {currency_code} не найдена.\n\n"
                f"📋 Все доступные валюты:\n{currencies_list}{get_commands_text()}"
            )
    else:
        await message.answer(f"❌ Ошибка при получении данных с сайта ЦБ РФ{get_commands_text()}")
...
async def cmd_compare(message: types.Message):
    ...
    if rates:
        if currency1 in rates and currency2 in rates:
            ...
            await message.answer(f"{result}{get_commands_text()}")
        else:
            missing = []
...
async def cmd_date(message: types.Message):
    ...
    if not args:
        ...
        await message.answer(
            ...
            f"Чтобы сбросить дату, используйте: /date reset{get_commands_text()}"
        )
        return
    ...
    if date_input == "reset":
        selected_date = None
        await message.answer(f"✅ Дата сброшена. Теперь используется текущая дата.{get_commands_text()}")
        return
    ...
    if not re.match(date_pattern, date_input):
        await message.answer(
            ...
            f"Пример: 02/03/2002{get_commands_text()}"
        )
        return
    ...
    try:
        ...
        await message.answer(f"✅ Дата установлена: {selected_date}{get_commands_text()}")
    except ValueError:
        await message.answer(f"❌ Неверная дата! Проверьте правильность введенной даты.{get_commands_text()}")
...
async def cmd_chart(message: types.Message):
    ...
    if not args:
        await message.answer(
            ...
            f"Пример: /chart USD\n\n"
            f"Показывает изменение курса за последние 7 дней{get_commands_text()}")
        return
    ...
    for i in range(7):
        ...
        if rates and currency_code in rates:
            ...
            normalized = currency['value'] / currency['nominal']
    ...
    if values:
        ...
        for i, (date, value) in enumerate(zip(dates, values)):
            bar_length = int((value - min_val) / diff * 20)
            bar = "█" * bar_length
            chart_lines.append(f"{date}: {value:.4f} {bar}")
        
        result = (
            f"📈 График курса {currency_code} за последние 7 дней:\n\n" +
            "\n".join(reversed(chart_lines)) +
            f"\n\nМин: {min_val:.4f} RUB\nМакс: {max_val:.4f} RUB"
        )
        await message.answer(f"{result}{get_menu_text()}")
    else:
        await message.answer(f"❌ Не удалось получить данные для валюты {currency_code}{get_commands_text()}")
...
async def handle_text(message: types.Message):
    ...
    if len(parts) == 2 and all(len(part) == 3 and part.isalpha() for part in parts):
        ...
        if rates:
            ...
            await message.answer(f"❌ Ошибка при получении данных с сайта ЦБ РФ{get_menu_text()}")
    ...
    # Если пользователь просто написал код валюты (например: USD)
        ...
        await message.answer(
            ...
            f"Введите /help для справки.{get_commands_text()}"
        )
