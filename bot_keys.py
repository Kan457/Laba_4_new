...
#график сравнивает последние изменения валюты за 7 дней
#чтобы было меню в котором можно было воспользоваться как кнопками  или команами
...
def get_menu_keyboard():
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [
            ...
            InlineKeyboardButton(text="📅 Установить дату", callback_data=MenuCallback(action="date").pack())
        ],
        [
...
async def cmd_chart(message: types.Message):
    ...
    if values:
        ...
        await message.answer(f"❌ Не удалось получить данные для валюты {currency_code}", reply_markup=get_menu_keyboard())
